...
$usersWeekQ = $db->query("SELECT email FROM users WHERE last_login > ?",array($week));
$weekCount = $usersWeekQ->count();

$usersMonthQ = $db->query("SELECT email FROM users WHERE last_login > ?",array($month));
...
<div class="row "> <!-- rows for Info Panels -->
	...
	<table class="table">
	<thead><tr><th>email</th><th>[IP]</th></tr></thead>
	<tbody>
	<?php foreach($usersDay as $v1){ ?>
		<tr><td><a href="admin_user.php?id=<?=$v1->id?>"><?=ucfirst($v1->email)?></a></td><td>[0.0.0.0]</td></tr>
	<?php } ?>
	</tbody>
	</table>
	</div>
